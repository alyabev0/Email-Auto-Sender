    ...
    <React.Fragment>
      ...
      <Flex w="80%" minH="100%" direction="column" alignItems="center">
        <Flex justifyContent="center" alignItems="center" mt="4%">
          <Text fontWeight="300" fontSize="50px" textAlign="center">
            Шаблоны писем
          </Text>
        </Flex>
        <Flex
        ...
        >
          ...
          <Select placeholder="Выбрать шаблон" maxW="40%">
            <option
              onClick={(event) => templateChoice(event)}
              id="1"
              value={template.firstContact}
            >
              Первый контакт
            </option>
            <option
              onClick={(event) => templateChoice(event)}
              id="2"
              value={template.interviewInvite}
            >
            ...
            <option
              onClick={(event) => templateChoice(event)}
              id="5"
              value={template.requestTA}
            >
              Письмо с ТЗ
            </option>
          </Select>

          <Button w="40%" onClick={() => reload()}>
            Reload
          </Button>
        </Flex>
        <Flex
